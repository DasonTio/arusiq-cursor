...
const METRIC_COMPONENTS = ['Metric', 'KpiTile', 'MetricValue', 'ChartCard', 'SavingsChart', 'CarbonCard'];
...
for (const file of files) {
  ...
  lines.forEach((raw, i) => {
    ...
    for (const c of METRIC_COMPONENTS) {
      ...
      if (!/\bprovenance\s*=/.test(chunk))
        add(file, n, 'metric-needs-provenance',
    ...
    if (/\bcredits?\b/i.test(jsxText) && !/creditCard|credit_card/i.test(raw))
      add(file, n, 'forbidden-word-credit',
  ...
  if (/\bnoData\b/.test(src) && /(\?\?|\|\|)\s*0\b/.test(src))
    add(file, 0, 'missing-is-not-zero',
...
console.log(`  ✓ provenance — ${files.length} file(s): every figure declares its origin`);
